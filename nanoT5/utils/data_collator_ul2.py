# Modified from: https://github.com/catie-aq/flashT5/blob/main/src/data/data_collator_ul2.py
...
class DataCollatorForUL2MLM(DataCollatorMixin):
    ...
    def __init__(
        ...
        self.prefixes = [
            tokenizer.encode(denoiser_list[i]["prefix"], return_tensors="np").flatten()[
                :-1
            ]
            for i in range(len(denoiser_list))
        ]
        ...
        self.extra_ids = sorted(
            [
                ...
                for i, x in enumerate(tokenizer.all_special_tokens)
                if "extra" in x
            ],
            reverse=True,
        )
        ...
        self.denoiser_optimal_len = [
            self.compute_input_and_target_lengths(
                max_length - max_prefix_len, x["r"], x["mu"]
            )
            for x in self.denoiser_list
        ]
            ...
            for i, x in enumerate(input_ids):
                if included_elements[i] == False:
                    ...
                    if concatenated_inputs is None:
                        ...
                        concatenated_inputs.shape[1] + size_inputs < self.max_length
                    ) and (
                        concatenated_labels.shape[1] + size_labels
                        ...
                        concatenated_inputs = np.concatenate(
                            [concatenated_inputs, x], axis=1
                        )
                        concatenated_labels = np.concatenate(
                            [concatenated_labels, item_labels], axis=1
                        )
                        included_elements[i] = True
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> BatchEncoding:
        ...
        if not has_length:
            for i in range(len(examples)):
                # print(i)
                examples[i]["length"] = len(examples[i]["input_ids"])
        ...
        causal_labels[causal_labels == self.tokenizer.pad_token_id] = (
            -100
        )  # ignore padding indices for the loss
    ...
    def filter_input_ids(self, input_ids, sentinel_ids, prefixes=None, with_eos=True):
        ...
        # Remove EOS tokens and negative values
        mask = (input_ids != self.tokenizer.eos_token_id) & (input_ids >= 0)
        input_ids = input_ids[mask].reshape(batch_size, -1)

        # print(f"Filtered input_ids shape: {input_ids.shape}")

        if prefixes is not None:
            # Ensure prefixes is 2D and has the same batch size as input_ids
            if prefixes.ndim == 1:
                prefixes = np.repeat(prefixes.reshape(1, -1), batch_size, axis=0)
            elif prefixes.shape[0] == 1:
                prefixes = np.repeat(prefixes, batch_size, axis=0)
            # print(f"Prefixes shape: {prefixes.shape}")
            input_ids = np.concatenate([prefixes, input_ids], axis=1)
            # print(f"Shape after adding prefixes: {input_ids.shape}")

        if with_eos:
            eos_tokens = np.full(
                (batch_size, 1), self.tokenizer.eos_token_id, dtype=np.int32
            )
            input_ids = np.concatenate([input_ids, eos_tokens], axis=1)
            # print(f"Final shape after adding EOS: {input_ids.shape}")
